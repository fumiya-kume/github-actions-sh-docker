...
This container registers a GitHub Actions self-hosted runner and begins processing jobs.

Required environment variables:
  RUNNER_URL         The URL of the GitHub repository or organization (e.g., https://github.com/owner/repo).
  RUNNER_TOKEN       The token for registering the runner. Alternatively, provide GITHUB_RUNNER_PAT.
...
Optional environment variables:
  GITHUB_RUNNER_PAT  A GitHub Personal Access Token with permissions to retrieve a runner token 
                     (required if RUNNER_TOKEN is not provided).
  RUNNER_NAME        The name for the runner. Defaults to the container's hostname.
...
if [ -z "$RUNNER_URL" ]; then
  echo "Error: RUNNER_URL is not set." >&2
  show_help
  exit 1
fi
...
if [ -z "$RUNNER_TOKEN" ]; then
  if [ -z "$GITHUB_RUNNER_PAT" ]; then
    echo "Error: Neither RUNNER_TOKEN nor GITHUB_RUNNER_PAT is set. Provide one of them to register the runner." >&2
    show_help
    exit 1
  fi
  ...
  if [ -z "$RUNNER_TOKEN" ] || [ "$RUNNER_TOKEN" = "null" ]; then
    echo "Error: Failed to retrieve runner token automatically. Check your GITHUB_RUNNER_PAT and repository permissions." >&2
    exit 1
  fi
...
# Set a default runner name if not provided; default to container's hostname.
